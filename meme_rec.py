...
fingers = np.array([[2,3,4],[5,6,8],[9,11,12],[13,15,16],[17,19,20]]) # thumb, index, middle, ring e pinky
...
def meme1(): # frog
    if (axisX[4] <= axisX[8]+9 and axisX[4] >= axisX[8]-9) and axisX[4] != 0 and (axisY[4] <= axisY[8]+10 and axisY[4] >= axisY[8]-10) and axisY[4] != 0: # checks if the index is touching the thumb
        if calculate_angle(fingers[2], 2, axisX, axisY, axisZ) < 20 and calculate_angle(fingers[3], 3, axisX, axisY, axisZ) < 20 and calculate_angle(fingers[4], 4, axisX, axisY, axisZ) < 20: # checks if the other fingers are down
...
def meme2and3(): # billie and finger down
    if calculate_angle(fingers[1], 1, axisX, axisY, axisZ) < 20 and calculate_angle(fingers[2], 2, axisX, axisY, axisZ) < 20:# checks if the index finger and the middle finger are straight
        if calculate_angle(fingers[3], 3, axisX, axisY, axisZ) > 100 and calculate_angle(fingers[4], 4, axisX, axisY, axisZ) > 100: # checks if the others are curved
            if axisY[8] > axisY[5] and axisY[12] > axisY[9]: # checks if the finger is poiting down
...
def meme4and5(): # panic and thinking
    if calculate_angle(arms[0], 0, pose_axisX, pose_axisY, pose_axisZ)>120 or calculate_angle(arms[1], 1, pose_axisX, pose_axisY, pose_axisZ)>120: # checks the arms angles
        if ((pose_axisX[16] <= pose_axisX[8]+20 and pose_axisX[16] >= pose_axisX[8]-20) and pose_axisX[16] != 0 and (pose_axisY[16] <= pose_axisY[8]+20 and pose_axisY[16] >= pose_axisY[8]-20) and pose_axisY[16] != 0) or ((pose_axisX[15] <= pose_axisX[7]+20 and pose_axisX[15] >= pose_axisX[7]-20) and pose_axisX[15] != 0 and (pose_axisY[15] <= pose_axisY[7]+20 and pose_axisY[15] >= pose_axisY[7]-20) and pose_axisY[15] != 0): # checks if the hand is close to the head
